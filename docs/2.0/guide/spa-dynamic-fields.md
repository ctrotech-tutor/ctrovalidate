...
When you initialize Ctrovalidate on a form, it automatically discovers all the fields that have the `data-ctrovalidate-rules` attribute. However, if you add a new field to the DOM *after* initialization, the validator won't know about it. That's where `addField()` comes in.
